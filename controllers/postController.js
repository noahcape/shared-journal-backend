...
module.exports = class PostsController {
    ...
    async publicGet(req, res) {
        let displayName = ""
        req.query.journal_name.split("_").map(name => { displayName += name + " " })
    ...
    async deletePost(req, res) {
        Post.findOne({ user: req.user, _id: req.query.id }).then(async (post) => {
            post.image_keys && post.image_keys.forEach(async key => {
    ...
    async editPost(req, res) {
        ...
        if (keysToDelete && imagesToDelete) {
            ...
            postToEdit.image_keys.forEach(key => {
                if (!keysToDelete.includes(key)) { imageKeys.push(key) }
            })

            postToEdit.images.forEach(image => {
                if (!imagesToDelete.includes(image)) { images.push(image) }
            })
        ...
        // update post
        await Post.replaceOne({ _id: req.query.id }, postToEdit).catch(err => { console.error(err) })

        // get updated post
        const updatedPost = await Post.findOne({ user: req.user, _id: req.query.id }).catch(err => { console.error(err) })
